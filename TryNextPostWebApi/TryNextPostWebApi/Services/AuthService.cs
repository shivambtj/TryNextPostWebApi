...

{
    public class AuthService : IAuthService
    {
       ...
       signingCredentials: credentials
        ...
        public async Task<Tuple<int, string>> ForgetPasswordAsync(string emailId)
        {
            try
            {
                var user = await _appDbContext.UserMasters.FirstOrDefaultAsync(x => x.EmailId == emailId);
                ...
                //================================= Generate OTP==================================
                string? OTP = RandomNumberGenerator.GetInt32(10000, 100000).ToString();
                ...
                //============================== Send Email=============================
                var mailResult = await _notificationService.SendOTPForMail(emailId, OTP,user.UserName);

                if (mailResult.Item1 == 1)
                {
                    await _appDbContext.SaveChangesAsync();
                    return new Tuple<int, string>(1, $"OTP sent successfully {emailId}");
                }
                else
                {
                    return new Tuple<int, string>(-1, "OTP  not sent successfully");
                }
        ...
        //=======================end Forget Password  =========================
        //=============================start verify OPT =================================================
        public async Task<Tuple<int,string>> verifyPassword(string otp)
        {
            try
            {
                var result = await _appDbContext.UserMasters.FirstOrDefaultAsync(x => x.ForgotPasswordOtp == otp);
                if(result==null)
                {
                    return new Tuple<int, string>(0, "otp not match");
                }
                if (DateTime.UtcNow > result.OtpExpiryTime)
                {
                    result.OtpExpiryTime = null;
                    result.ForgotPasswordOtp = null;
                    _appDbContext.Update(result);
                    await _appDbContext.SaveChangesAsync();
                    return new Tuple<int, string>(1, "OTP has expired.");
                }
                if(result.IsOtpVerified)
                {
                    return new Tuple<int, string>(2, "OTP Already Used.");
                }
                result.IsOtpVerified = true;
                _appDbContext.Update(result);
                await _appDbContext.SaveChangesAsync();
                return new Tuple<int, string>(3, "OTP verify sucessfully");
            }
            catch(Exception ex)
            {
                return new Tuple<int, string>(-1, ex.Message);
        ...
        public async Task<Tuple<int,string>> NewPassword(UserMasterDto userMasterDto)
        {
            try
            {
                var exitstUser= await _appDbContext.UserMasters.FirstOrDefaultAsync(x=>x.EmailId == userMasterDto.EmailId);
                if(exitstUser == null)
                {
                    return new Tuple<int, string>(0, "user not exits");
                }
                exitstUser.Password = PasswordHashing(userMasterDto);
                exitstUser.PasswordValidity = DateTime.UtcNow.AddMonths(6);
                _appDbContext.Update(exitstUser);
                await _appDbContext.SaveChangesAsync();
                return new Tuple<int, string>(1, "User Password Update Sucessfully");
            }
            catch(Exception ex)
            {
                return new Tuple<int, string>(-1, ex.Message);
